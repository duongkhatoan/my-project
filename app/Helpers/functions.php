...
if (!function_exists('load_info_province')) {
    function load_info_province($id)
    {
        $data = load_file_location();
        if ($data) {
            $data = $data->province;
...
if (!function_exists('load_info_district')) {
    function load_info_district($id)
    {
        $data = load_file_location();
        if ($data) {
            $data = $data->district;
...
if (!function_exists('load_info_ward')) {
    function load_info_ward($id)
    {
        $data = load_file_location();
        if ($data) {
            $data = $data->commune;
            $data = array_filter($data, function ($item) use ($id) {
                return $item->idCommune === $id; // Điều kiện lọc theo tên tỉnh
            });
            $data = array_values($data);
            return $data;
        } else {
